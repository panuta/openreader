    ...
    $('#publication-modal .save_button').live('click', function() {
        ...
        $.post('/ajax/' + var_organization_slug + '/publication/edit/', {uid:uid, title:title, description:description, tags:tagnames, classification:classification, weight:weight}, function(response) {
            if(response.status == 'success') {
                ...
                $('.filename .weight-' + weight).remove();
                $('#'+ uid +' .filename .label-weight').remove();
                $('#'+ uid +' .filename a').before('<span class="label label-success label-weight weight-'+ weight +'">'+ response.weight_display +'</span>');
            } else {
